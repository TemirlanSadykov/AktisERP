...
def set_warehouse_scope(request):
    if request.method == "POST":
        request.session["warehouse_scope_id"] = request.POST.get("warehouse_id", "all")
    return redirect(request.META.get("HTTP_REFERER") or reverse("admin_page"))

def set_supplier_scope(request):
    if request.method == "POST":
        request.session["supplier_scope_id"] = request.POST.get("supplier_id", "all")
    return redirect(request.META.get("HTTP_REFERER") or reverse("admin_page"))